using DataFrames, CSV, Tables, LinearAlgebra, Statistics, StatsBase, Random, Clustering, NaNMath, Base.Threads
...
function main()
    ...
    println("There are $(Threads.nthreads()) threads present.")
    df_raw = CSV.read(data_name, delim = " ", header=0)
    df_x = convert(Matrix, df_raw)
    x = df_x[:,1:p]
    bic_save = Dict()
    for ig in rg
        bic_save[ig] = fill!(zeros(maximum(rq), maximum(rm)), -Inf)
    end
    runs = collect(Iterators.product(rg, rq, rm))
    maxrun = length(runs)
    thread_job = zeros(length(runs),3)
    ...
    Threads.@threads for k in 1:maxrun
        ...
        qq = trunc(Int, thread_job[k,2])
        mm = trunc(Int, thread_job[k,3])
        @inbounds bic_save[gg][qq,mm] = work(x, gg, qq, mm, n, p)[1]
        #println("G = $gg Q = $qq M = $mm got BIC = $(bic_save[gg][qq,mm])")
    end
    #println(bic_save)
    bic_b = -Inf
    g_b = -Inf
    ...
    m_b = -Inf
    for ig in rg
        if maximum(bic_save[ig]) > bic_b
            bic_b = maximum(bic_save[ig])
            g_b = ig
            q_b = argmax(bic_save[ig])[1]
            m_b = argmax(bic_save[ig])[2]
        end
    end
    time_out = time()
    println("The best BIC = $bic_b for G = $g_b, Q = $q_b, and M = $m_b.")
...
#julia OMP_PGMM.jl 2 2 6 8 1 4 62 461 alon.txt
