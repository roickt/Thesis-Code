using DataFrames, CSV, Tables, LinearAlgebra, Statistics, StatsBase, Random, Clustering, NaNMath, MPI
...
function master(comm, size, rank)
    ...
    array_job = zeros(length(runs),5)
    for k in 1:length(runs)
        for q in 1:3
            array_job[k,q] = runs[k][q]
        end
        array_job[k,4] = n
        array_job[k,5] = p
    end
    tasks_all = length(runs)
    task_all_index = 0
    num_workers = size - 1
    closed_workers = 0
        ...
        if flag == 1
            ...
            if tag == 0
                if task_all_index < tasks_all
                    message_send = array_job[(task_all_index+1), :]
...
function slave(comm, size, rank)
    ...
    while go == true
        ...
        if tag == 3
            ...
            qq = trunc(Int, message_send[2])
            mm = trunc(Int, message_send[3])
            n = trunc(Int, message_send[4])
            p = trunc(Int, message_send[5])
            data = zeros(n*p)
            MPI.Recv!(data, 0, MPI.MPI_ANY_TAG, comm)
            x_df = reshape(data, n, p)
            #println("Worker $rank has g = $gg, q = $qq, and model = $mm.") # found bic = $(bic_save[9])")
            #bic_save = pgmmEM(df_alon, gg:gg, qq:qq, Nothing, false, 2, true, 3, Nothing, [tt], 123456, 0.1, false)
            res = work(x_df, gg, qq, mm, n, p)
