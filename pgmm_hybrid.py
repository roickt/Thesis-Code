...
def work(x, gg, qq, mm, n, p, seed, index, z_save, rank):
    ...
    if not math.isnan(temp[1]):
        ...
        bic_out = -np.Inf
    z_save[index] = [gg, qq, (mm + 1), bic_out, z_best]
...
def master(comm, size, rank, status):
    ...
    while closed_workers < num_workers:
        ...
        if tag == 0:
            if q_index < q_max:
                send_count += 1
                message_send = [g_min, g_max, m_min, m_max, rq[q_index-1], n, p, seeder]
            ...
            #print("G = ", g_temp, " Q = ", q_temp, " Model = ", m_temp, " BIC = ", bic_temp)
            #print("Bic save is ", bic_save, "New Bic above is better: ", (message_recv[0] > bic_save))
            if message_recv[0] > bic_save and message_recv[1] != 0:
                #print("Updating BIC Save ", bic_save, "to ", message_recv[0])
                bic_save = bic_temp
                g_b = g_temp
                q_b = q_temp
                m_b = m_temp
    ...
    while go == True:
        ...
        if tag == 3:
           ...
           # multiprocessing.set_start_method('spawn')
            ...
            z_save = mgr.dict()
            #z_save = {}
            print('Calling Python multiprocessing process from Python MPI rank {}-{}'.format(rank, qq))
            jobs = [multiprocessing.Process
                    (target=work, args=(x_df, gqm[0], qq, gqm[1], n, p, seed, gqm[2], z_save, rank))
            ...
            ind_save = 0
            for ind in index:
                if z_save[ind][3] > bic_save2:
                    ind_save = ind
                    bic_save2 = z_save[ind][3]
            message_recv[0] = bic_save2
            message_recv[1] = int(z_save[ind_save][0])
            message_recv[2] = int(z_save[ind_save][1])
            message_recv[3] = int(z_save[ind_save][2])
            comm.send(message_recv, dest=0, tag=1)
            message_final_z = (z_save[ind_save][4]).flatten()
